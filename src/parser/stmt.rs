...
impl<'a> Parser<'a> {
    pub fn parse_block(&mut self) -> Statements {
        self.expect_tok_recover(Token::OpenBracket);
        let code = self.parse_statements();
        self.expect_tok_recover(Token::ClosedBracket);

        code
    }

    pub fn parse_statement(&mut self) -> StmtNode {
        ...
        let typ = match self.peek() {
            ...
            Token::For => {
                ...
                let iter = match self.parse_pattern() {
                    Ok(p) => p,
                    Err(_) => todo!("add more context?"),
                };

                self.expect_tok_recover(Token::In);
            ...
            Token::Try => {
                ...
                self.expect_tok_recover(Token::Catch);
    ...
    pub fn parse_statements(&mut self) -> Statements {
