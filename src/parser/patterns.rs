...
use super::ast::pattern::PatternNode;
use super::{ParseResult, Parser};
use crate::lexer::tokens::Token;
use crate::list_helper;
use crate::parser::ast::pattern::{AssignPath, PatternType};
    ...
    fn parse_pattern_value(&mut self) -> ParseResult<PatternNode> {
        ...
        let pat = 'out_pat: {
            match self.next()? {
                ...
                Token::Ident | Token::Slf => {
                    ...
                    PatternType::Path { var: name, path }
                },
                ...
                Token::TypeIndicator => {
                    ...
                    if self.skip_tok(Token::DoubleColon)? {
                        ...
                        let map = dictlike_destructure!();
                        PatternType::InstanceDestructure(typ, map)
                    } else if self.skip_tok(Token::Arrow)? {
                        let ret = self.parse_pattern()?;
                        PatternType::MacroPattern {
                            args: vec![PatternNode {
                                pat: Box::new(PatternType::Type(typ)),
                                span,
                            }],
                            ret,
                        }
                    } else {
                        PatternType::Type(typ)
                    }
                },
                Token::Eq => {
                ...
                Token::LParen => {
                    if self.skip_tok(Token::RParen)? {
                        if self.skip_tok(Token::Arrow)? {
                            let ret = self.parse_pattern()?;
                            PatternType::MacroPattern { args: vec![], ret }
                        } else {
                            PatternType::Empty
                        }
                    } else {
                        ...
                        if self.skip_tok(Token::Comma)? {
                            ...
                            let ret = self.parse_pattern()?;
                            PatternType::MacroPattern { args, ret }
                        } else {
                            self.expect_tok(Token::RParen)?;
                            if self.skip_tok(Token::Arrow)? {
                                let ret = self.parse_pattern()?;
                                PatternType::MacroPattern { args: vec![p], ret }
                            } else {
                                *p.pat
                            }
                        }
                ...
                Token::QMark => PatternType::MaybeDestructure(None),
        ...
        loop {
            ...
            let pat = match self.peek_strict()? {
                Token::QMark => {
                    self.next()?;
                    PatternType::MaybeDestructure(Some(node))
                },
                Token::LSqBracket => {
                    self.next()?;
                    if self.skip_tok(Token::RSqBracket)? {
                        ...
                        self.expect_tok(Token::RSqBracket)?;
                        PatternType::ArrayPattern(node, pat)
                    }
                },
                Token::LBracket
            ...
            left = PatternNode {
                ...
                pat: Box::new({
                    let p = match peek {
                        ...
                        Token::BinOr => PatternType::Either,
                        Token::BinAnd => PatternType::Both,
                        _ => unreachable!(),
                    };
                    p(left, right)
                }),
        ...
        loop {
            ...
            let pat = match self.peek_strict()? {
                Token::If => {
                    ...
                    PatternType::IfGuard { pat: left, cond }
                },
                _ => {
                    break;
                },
